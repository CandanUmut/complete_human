...
const BADGES: Record<RoadmapTopic['evidence'], { dot: string; label: string }> = {
  strong: { dot: '#4a8a5c', label: 'Strong evidence' },
  moderate: { dot: '#C98B6B', label: 'Moderate evidence' },
  emerging: { dot: '#C9A86B', label: 'Emerging evidence' },
  consensus: { dot: '#888', label: 'Expert consensus' },
};
...
export function RoadmapView() {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="h-title">The Roadmap</h1>
        <p className="opacity-70 text-sm mt-1">The evidence behind every practice. Eight interdependent layers, drawn from the research.</p>
      </div>
      ...
      <div className="card p-3 flex items-center gap-2">
        ...
        <input
          ...
          placeholder="Search topics, findings, researchers…"
          value={query}
      ...
      {filtered.map((layer) => (
        <section key={layer.layer} className="space-y-3">
          <div className="flex items-center gap-2">
            ...
            <h2 className="h-section">{layer.title}</h2>
          </div>
...
function TopicCard({ topic, color, open, onToggle }: { topic: RoadmapTopic; color: string; open: boolean; onToggle: () => void }) {
  ...
  const badge = BADGES[topic.evidence];
    ...
    <article className="card overflow-hidden">
      ...
      {open && (
        <div className="px-5 pb-5 space-y-4 fade-in">
          <Section title="Why it matters" color={color}>{topic.why}</Section>
          <Section title="What to develop" color={color}>{topic.what}</Section>
          <div>
            <SectionTitle color={color}>How to start</SectionTitle>
          ...
          {topic.resources.length > 0 && (
            <div>
              <SectionTitle color={color}>Go deeper</SectionTitle>
          ...
          {topic.practiceIds && topic.practiceIds.length > 0 && (
            <div>
              <SectionTitle color={color}>Add to my practice</SectionTitle>
